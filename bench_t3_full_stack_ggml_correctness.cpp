...
std::string fixture_dir() {
    const char * env = std::getenv("GGML_T3_FULL_STACK_FIXTURE_DIR");
    if (env != nullptr && env[0] != '\0') {
        return env;
    }
    return "/root/chatterbox/exports/ggml_t3_full_stack_fixture_p" + std::to_string(PAST_LEN);
}

std::string output_json() {
    return "/root/chatterbox/exports/benchmarks/ggml_t3_full_stack_correctness_p" + std::to_string(PAST_LEN) + "_2026-07-08.json";
}

std::string output_md() {
    return "/root/chatterbox/exports/benchmarks/ggml_t3_full_stack_correctness_p" + std::to_string(PAST_LEN) + "_2026-07-08.md";
}
...
struct StackGraph {
    ...
    ggml_tensor * hidden_pre_ln_f = nullptr;
...
ggml_tensor * build_layer(ggml_context * ctx, ggml_tensor * cur, const LayerTensors & l) {
...
StackGraph build_graph() {
    ...
    for (int i = 0; i < N_LAYER; ++i) {
        ...
        cur = build_layer(ctx, cur, g.layers.back());
    }

    g.hidden_pre_ln_f = cur;
    g.ln_f_w = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, HIDDEN);
    g.ln_f_b = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, HIDDEN);
    g.final_hidden = layer_norm(ctx, g.hidden_pre_ln_f, g.ln_f_w, g.ln_f_b);
...
void load_layer(LayerTensors & l, int i) {
    char prefix[32];
    std::snprintf(prefix, sizeof(prefix), "layer_%02d", i);
    const std::string p(prefix);
    set_tensor_file(l.input_past_k, p + "_past_k", HEAD_DIM * PAST_LEN * N_HEAD);
    set_tensor_file(l.input_past_v, p + "_past_v", PAST_LEN * HEAD_DIM * N_HEAD);
...
void load_fixture(StackGraph & g) {
    set_tensor_file(g.input, "input_hidden", HIDDEN);
    for (int i = 0; i < N_LAYER; ++i) {
        load_layer(g.layers[i], i);
    }
...
Result run_backend(
        ...
        const std::vector<float> & ref_hidden,
        const std::vector<float> & ref_final_hidden,
    ...
    result.expected_argmax = argmax(ref_logits);
    ...
    try {
        ...
        const int repeats = label == "vulkan" ? 10 : 5;
        ...
        result.mean_ms = elapsed_ms / static_cast<double>(repeats);

        std::vector<float> actual_hidden(HIDDEN);
        std::vector<float> actual_final_hidden(HIDDEN);
        std::vector<float> actual_logits(SPEECH_VOCAB);
        ggml_backend_tensor_get(g.hidden_pre_ln_f, actual_hidden.data(), 0, actual_hidden.size() * sizeof(float));
        ggml_backend_tensor_get(g.final_hidden, actual_final_hidden.data(), 0, actual_final_hidden.size() * sizeof(float));
        ggml_backend_tensor_get(g.logits, actual_logits.data(), 0, actual_logits.size() * sizeof(float));

        result.hidden = compare_arrays(actual_hidden, ref_hidden);
        result.final_hidden = compare_arrays(actual_final_hidden, ref_final_hidden);
        result.logits = compare_arrays(actual_logits, ref_logits);
        result.actual_argmax = argmax(actual_logits);
        result.top10_overlap = overlap(topk(actual_logits, 10), topk(ref_logits, 10));
        result.ok = result.actual_argmax == result.expected_argmax && result.top10_overlap >= 8 && result.logits.allclose_1e_2;
...
void write_comparison_json(std::ofstream & f, const char * name, const Comparison & c, const char * indent) {
    f << indent << "\"" << name << "\": {\n";
    f << indent << "  \"max_abs_error\": " << c.max_abs_error << ",\n";
    f << indent << "  \"mean_abs_error\": " << c.mean_abs_error << ",\n";
    f << indent << "  \"rms_error\": " << c.rms_error << ",\n";
    f << indent << "  \"allclose_1e_4\": " << (c.allclose_1e_4 ? "true" : "false") << ",\n";
    f << indent << "  \"allclose_1e_3\": " << (c.allclose_1e_3 ? "true" : "false") << ",\n";
    f << indent << "  \"allclose_1e_2\": " << (c.allclose_1e_2 ? "true" : "false") << "\n";
    f << indent << "}";
}
...
void write_outputs(const std::vector<Result> & results) {
    {
        ...
        f << "{\n";
        f << "  \"description\": \"Full real Chatterbox T3 GPT-2 cached one-token ggml correctness probe\",\n";
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            ...
            f << "      \"mean_ms\": " << r.mean_ms << ",\n";
            f << "      \"repeats\": " << r.repeats << ",\n";
            f << "      \"expected_argmax\": " << r.expected_argmax << ",\n";
            f << "      \"actual_argmax\": " << r.actual_argmax << ",\n";
            f << "      \"top10_overlap\": " << r.top10_overlap << ",\n";
            write_comparison_json(f, "hidden_pre_ln_f", r.hidden, "      ");
            f << ",\n";
            write_comparison_json(f, "final_hidden", r.final_hidden, "      ");
            f << ",\n";
            write_comparison_json(f, "logits", r.logits, "      ");
            f << ",\n";
...

    {
        ...
        f << "# ggml Full T3 Stack Correctness Probe\n\n";
        f << "Fixture: `" << fixture_dir() << "/manifest.json`\n\n";
        f << "Cache length: `" << PAST_LEN << "` past tokens, `" << SEQ_LEN << "` tokens after update.\n\n";
        f << "| Backend | Device | Mean ms | Repeats | Logits max abs | Logits allclose 1e-3 | Argmax | Top10 overlap | Status |\n";
        f << "|---|---|---:|---:|---:|---:|---:|---:|---|\n";
        for (const auto & r : results) {
            f << "| " << r.backend
              ...
              << " | " << r.mean_ms
              << " | " << r.repeats
              << " | " << r.logits.max_abs_error
              << " | " << (r.logits.allclose_1e_3 ? "true" : "false")
              << " | " << r.actual_argmax << "/" << r.expected_argmax
              << " | " << r.top10_overlap << "/10"
              << " | " << (r.ok ? "ok" : r.error)
              << " |\n";
        }
...
int main() {
    ...
    std::vector<float> ref_hidden = read_f32(path_for("ref_hidden_pre_ln_f"), HIDDEN);
    std::vector<float> ref_final_hidden = read_f32(path_for("ref_final_hidden"), HIDDEN);
    std::vector<float> ref_logits = read_f32(path_for("ref_logits"), SPEECH_VOCAB);

    std::vector<Result> results;
    results.push_back(run_backend("cpu", cpu, ref_hidden, ref_final_hidden, ref_logits));
    results.push_back(run_backend("vulkan", vk, ref_hidden, ref_final_hidden, ref_logits));
    ...
    for (const auto & r : results) {
        std::cout << r.backend
                  << " device=\"" << r.device << "\""
                  << " mean_ms=" << r.mean_ms
                  << " logits_max_abs_error=" << r.logits.max_abs_error
                  << " argmax=" << r.actual_argmax << "/" << r.expected_argmax
                  << " top10=" << r.top10_overlap << "/10"
